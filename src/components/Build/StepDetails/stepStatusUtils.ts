...
export const getStatusIcon = (status: Step["status"]): ReactElement => {
  switch (status) {
    case "completed":
      return createElement(CheckCircle, { className: "w-5 h-5 text-green-600 dark:text-green-400" });
    case "in-progress":
      return createElement(Clock, { className: "w-5 h-5 text-orange-600 dark:text-orange-400" });
    default:
